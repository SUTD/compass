...
Cooper::Cooper(CNode* _c, Term* t)
{
	...
	if(DEBUG) {
		cout << "Original constraint: " << c->to_string() << endl;
	}

	remove_coefficients();

	if(DEBUG) {
		cout << "After normalizing: " << c->to_string() << endl;
	}
...
CNode* Cooper::normalize_coefficients(CNode* c, bignum lcm)
{
	...
	if(c->get_type() == EQ)
	{
		...
		if(!eq->contains_term(elim_t)) return eq;
		Term* rhs = eq->get_rhs();
		Term* lhs = eq->get_lhs();

		Term* other = NULL;
		if(rhs == elim_t) other = lhs;
		else if(lhs == elim_t) other = rhs;
		assert(other!=NULL);

		Term* new_t = other->multiply(lcm.to_int());
		CNode* new_leaf = ILPLeaf::make(new_t, elim_t, ILP_EQ);
		return new_leaf;
	...
	if(c->get_type() == MOD)
	{
		...
		if(t->get_term_type() == ARITHMETIC_TERM)
		{
			...
			long int new_c =  at->get_constant()*mult_factor.to_int();
			Term* new_at = ArithmeticTerm::make(new_elems, new_c);
			long int new_constant = (mult_factor*mod_c).to_int();
...
void Cooper::compute_a_and_b_terms(CNode* c, bool in_neg)
{
	...
	if(c->get_type() == ILP)
	{
		ILPLeaf* ilp = static_cast<ILPLeaf*>(c);
		if(!ilp->contains_elem(elim_t)) return;
		long int coef = ilp->get_coefficient(elim_t);
