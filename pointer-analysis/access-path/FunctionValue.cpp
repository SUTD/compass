...
{
	cout << "_make_division ap types " << ap1->to_string() << endl << ap2->to_string() << endl
			<< ap1->get_ap_type() << "} {" <<ap2->get_ap_type() <<endl;
...
{
	//cout << "inside FunctionValue constructor " << endl;
	...
	//cout << "out of FunctionValue constructor " << endl;
}
...
{
	bool arg1_parenthesis = false;
	bool arg2_parenthesis = false;
	if((arguments[0]->get_ap_type() == AP_ARITHMETIC)
			||(arguments[0]->get_ap_type() == AP_FUNCTION)){
		arg1_parenthesis = true;
	}
	if((arguments[1]->get_ap_type() == AP_ARITHMETIC)
			||(arguments[1]->get_ap_type() == AP_FUNCTION)){
		arg2_parenthesis = true;
	}
	...
	string arg1_content = arguments[0]->to_string();
	string arg1;
	if(arg1_parenthesis){
		arg1 = "(";
		arg1_content += ")";
	...
	string arg2_content = arguments[1]->to_string();
	string arg2;
	if(arg2_parenthesis){
		arg2 = "(";
		arg2_content += ")";
...
bool FunctionValue::contains_term(AccessPath* term){
	for(vector<AccessPath*>::iterator it = arguments.begin(); it != arguments.end(); it++){
		if(*it == term)
			return true;
	}
	return false;
...
Term* FunctionValue::strip_term(Term*& t){
	if(!contains_term( AccessPath::to_ap(t))){
		cout << "Function Value don't have term " << t->to_string() << endl;
		return this->to_term();
	}
	cout << "Function Value have term " << t->to_string() << endl;
	cout << "argument[0]" << arguments[0]->to_term()->to_string() << endl;
	cout << "argument[1]" << arguments[1]->to_term()->to_string() << endl;
	if(arguments[0] == AccessPath::to_ap(t)){
		return arguments[1]->to_term();
	}else if(arguments[1] == AccessPath::to_ap(t)){
		return arguments[0]->to_term();
	}
	else
		assert(false);
}
