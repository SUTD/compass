...
AccessPath* AccessPath::get_ap_from_symbol(sail::Symbol* s)
{
	...
	//cout << "Symbol is " << s->to_string() << endl;
	assert(s != NULL);
	if(s->is_constant()){
...
//	if((v->get_type() == var->get_type())
//			&& (v->is_argument() == var->get_original_var()->is_argument())
//			&& (v->is_global() == var->get_original_var()->is_global())
//			&& (v->get_arg_number() == var->get_original_var()->get_arg_number())
//			&& (v->is_return_variable() == var->is_return_var()))
//	return var;
//
//	v->set_name(v->get_var_name()+ "_" +int_to_string(idx++));
//
//	Variable* v_final = Variable::make_program_var(v);
