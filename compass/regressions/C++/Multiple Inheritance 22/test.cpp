...
public:
  ...
  Student* get_ith_member(int i)
  {
    if(i<0 || i>= num_members) return NULL;
    return students[i];
  }
...
void test()
{
  ...
  Student* mary = new PhDStudent(BIO, "enzymes", stanford);
  Student* dave = new Undergraduate(3, ENGLISH, stanford);
  ...
  static_assert(sg->get_num_members() == 3);
  ...
  */
  for(int i=0; i< sg->get_num_members(); i++)
  {
    Student* s = sg->get_ith_member(i);
    static_assert(s->get_university() == stanford); //correct
  }
