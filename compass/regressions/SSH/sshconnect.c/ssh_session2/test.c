    ...
    u_int window, u_int maxpack, int extusage, char *remote_name, int nonblock)
{
	...
	assume(buffer_size(channels) == channels_alloc*sizeof(*channels));
	if(channels_alloc > 0) assume(channels!=NULL);
	...
	if (found < 0) {
		...
		channels = xrealloc(channels, (channels_alloc + 10) * sizeof(Channel *));
		channels_alloc += 10; 
	...
	channels[found] = c = xmalloc(sizeof(Channel));
	buffer_init(&c->input);
...
ssh_init_forwarding(void)
{
	...
	assume(options.num_local_forwards <= SSH_MAX_FORWARDS_PER_DIRECTION);
	assume(options.num_remote_forwards <= SSH_MAX_FORWARDS_PER_DIRECTION);
