//SSH/sshconnect.c/try_agent_authentication
...
Key *
ssh_get_next_identity(AuthenticationConnection *auth, char **comment)
{
	int keybits;
	u_int bits;
	u_char *blob;
...
Key *
ssh_get_first_identity(AuthenticationConnection *auth, char **comment)
{
	/* get number of identities and return the first entry (if any). */
	if (foo())
		return ssh_get_next_identity(auth, comment);
	return NULL;
}
 ...
 */
static int
try_agent_authentication(int** p, int* x)
{
	int type;
	char *comment;
	...
	key = ssh_get_first_identity(auth, &comment);
	if(key != NULL) key->rsa->n = 5;
