...
static unsigned int sessions_nalloc;
...
{
	Session *s, *tmp;
	assume(sessions_first_unused >= -1);
	assume(sessions!=NULL);
	assume(buffer_size(sessions) == sessions_nalloc*sizeof(*sessions));
	...
	static_assert(sessions_first_unused >=0);
