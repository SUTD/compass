...
do_ssh1_kex(void)
{
	...
	assume(sensitive_data.ssh1_host_key != NULL);
	assume(sensitive_data.server_key != NULL);
	assume(sensitive_data.server_key->rsa != NULL);
	assume(sensitive_data.ssh1_host_key->rsa != NULL);
	assume(sensitive_data.host_keys != NULL);
	assume(buffer_size(sensitive_data.host_keys) == options.num_host_key_files * sizeof(Key*));
