...
c_genericize (tree fndecl)
{
  ...
  /* COMPASS connector START
   * Only process all decls if there are no errors in source file.
   */
  if(!(errorcount || sorrycount))
  {
    if(output_sail_il){
    	set_sail_output_dir(output_sail_il);
    	process_fundecl_body(fndecl);
    }
  }
  /* COMPASS connector END */
